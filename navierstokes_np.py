...
def extend(w_interior, geo):
    ...
    '''
    w = zeros([4, Ni+2, Nj+2])    # extend boundary by one cell
    w[:,1:-1,1:-1] = w_interior.reshape([4, Ni, Nj])


    # inlet
    rho, u, v, E, p = primative(w[:,1,1:-1])    # boundary values
    c2 = 1.4 * p / rho
    c = sqrt(c2)
    mach2 = (u**2+v**2) / c2    # why only depend on u, not v?
...
for i in range(500):
    print('i = ', i, 't = ', t)
    w = opt.newton_krylov( lambda w: ns_kec(w, w0, geo, dt), w0 )
    if w._n_Newton == 1:
        break
    elif w._n_Newton < 5:
        w0 = w
