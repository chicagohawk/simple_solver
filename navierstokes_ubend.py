...
for i in range(100):
    print('i = ', i, 't = ', t)
    coef = array(1.+1e-5)
    w = solve(ns_kec, w0, args=(w0, geo, dt, coef), rel_tol=1E-7, abs_tol=1E-6)
    if w._n_Newton == 1:
        break
    elif w._n_Newton < 5:
        w0 = w
...
err_r = linalg.norm(w[:1600] - w_standard[:1600],2)
err_u = linalg.norm(w[1600:3200] - w_standard[1600:3200],2)
err_v = linalg.norm(w[3200:4800] - w_standard[3200:4800],2)
err_E = linalg.norm(w[4800:] - w_standard[4800:],2)
...
print (err_r, grad_r*1e-5) 
print (err_u, grad_u*1e-5)
print (err_v, grad_v*1e-5)
print (err_E, grad_E*1e-5)
