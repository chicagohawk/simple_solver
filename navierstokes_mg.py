...
def vis(w, geo):
    ...
    import numpy as np
    rho, u, v, E, p = primative(extend(w, geo))
    x, y = geo.xy
    xc, yc = geo.xyc
...
# ------------------ configure stdout -----------------
#stdout = sys.stdout    # handle on real stdout
#sys.stdout = StringIO.StringIO()    # redirect
...
# ======================= 2^n-by-2^n grid ======================#
for multigrid in range(4):
    print '====== mg: ', multigrid

    if geometry == 'bend':
        Ni, Nj = Ni*refine, Nj*refine
        theta = linspace(0, pi, Ni/2+1)
        r = 15 + 5 * linspace(-np.pi/2, np.pi/2, Nj+1)
    ...
    if w0 is None:
      ...
      w = reshape(w0, [4, Ni*Nj/refine/refine])
      ws = []
      for dim in range(4):
          ws.append( kron(w[dim].reshape([Ni/refine,Nj/refine]), 
                          ones([refine, refine])) )
      w = vstack(ws).reshape(4, Ni, Nj)
      w0 = ravel(w)
    
    for i in range( int( 100 * 4**(3-multigrid) ) ):
        print('i = ', i, 't = ', t)
        with redirect(StringIO.StringIO()) as new_stdout:
            w = newton_krylov(lambda w: ns_kec(w,w0,geo,dt), w0, verbose=True,
                inner_maxiter=20, outer_k=10)
        new_stdout.seek(0)
        verbose = new_stdout.read().split('\n')
        for line in range(len(verbose))[::-1]:
            try:
                _n_Newton = int( verbose[line].split(':')[0] )
                break
            except ValueError:
                continue
        if _n_Newton == 1:
...
print('Final, t = inf')
dt = t*5
w = newton_krylov(lambda w: ns_kec(w,w0,geo,dt), w0, verbose=True, inner_maxiter=50,
          outer_k=50)
set_trace()
